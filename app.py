...
class RichLogApp(App):
    def compose(self) -> ComposeResult:
        with Vertical():
            self.top_log = RichLog(highlight=True, markup=True)
            self.bottom_log = RichLog(highlight=True, markup=True)
            yield self.top_log
            # yield Rule(orientation=)
            yield self.bottom_log
    ...
    def on_key(self, event: events.Key) -> None:
        ...
        self.top_log.write(event)
        self.bottom_log.write(event)
